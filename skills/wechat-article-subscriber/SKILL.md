...
Configuration is pure dialogue: loop on `manage next` — it returns the current stage, the exact question to ask the user, and the command to run — until it reports `ready`. Ask exactly one question per turn: take the current question from the wizard, present it alone, wait for the answer, apply it, re-run the wizard, and only then ask the next question. Never batch multiple setup questions into one message or a multi-question form. Never hand the user a command list; secrets arrive in chat (after the retention warning), via the documented stdin pipe, or — for the Feishu App Secret — through the prepared local secret file, and are entered by the Agent. Inside the Feishu branch, drive `manage feishu-setup`, which confirms target choice (skip / map an existing table by pasting its URL via `manage feishu-target --url` / create the standard table after showing the field list) and supports the bot identity end-to-end without any user OAuth scan (bot creates the Base and grants the configured human manager full access).

1. Start with `manage next` (fallback: `manage status` for a read-only view). Read [references/setup.md](references/setup.md). Show the exact local file path, plaintext warning, required fields, and manifest once, then continue one question per turn. The remaining non-secret decisions are collected across successive turns, each as its own question: subscriptions (name plus WeChat alias), search window, Feishu skip/existing/create choice, identity, exact App ID, manager, target or Base/table names, and whether provisioning/sync are allowed. The bundled default roster at `assets/default_subscriptions.json` seeds the subscription list: when the subscription question is current, preview it with `manage subscriptions bulk-add --file assets/default_subscriptions.json --dry-run`, show it as the bounded companion of that single question, let the user strike entries and add their own, then apply by repeating the same command without `--dry-run`. Entries that duplicate configured subscriptions are skipped, roster aliases need no paid account search, and the user may later change the list with `manage subscriptions add/remove`. If the roster is missing or empty, collect subscriptions from the user directly. The Feishu destination is a required user decision: run `manage feishu-destination --mode skip|existing|create` with the user's answer, and never translate an omitted answer into `skip`. Do not restart completed setup steps.
...
4. When Feishu is selected, drive onboarding with `manage feishu-setup`: a deterministic state machine that returns the current stage, the exact question to ask the user, and the exact next command — a fresh user needs no app information prepared. Loop through identity → App → private profile → (bot: App Secret via the prepared local secret file + manager | user: one minimum Base authorization) → destination → target until it reports `run_feishu_validation`, then run `manage doctor --online`. The wizard emits every command (including the App Secret file flow); profile import, keychain remediation, and authorization-resume details live in [references/feishu.md](references/feishu.md).
